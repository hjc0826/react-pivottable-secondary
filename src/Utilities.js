...
import { isEqual, orderBy, sortBy, isEmpty, forOwn } from 'lodash';
...
const aggregatorTemplates = {
  ...
  fractionOf(wrapped, type = 'total', formatter = usFmtPct) {
    return (...x) =>
      function (data, rowKey, colKey, colName) {
        return {
          ...
          value() {
            return (
              this.inner.value() /
              data
                .getAggregator(...Array.from(this.selector || []), colName)
                .inner.value()
            );
          },
...
class PivotData {
  constructor(inputProps = {}) {
    ...
    this.aggregatorGathers = this.props.aggregatorGather.map(item => {
      return this.props.aggregators[item.aggregatorName](item.vals);
    })
  ...
  sortKeys() {
    if (!this.sorted) {
      ...
      if (!isEmpty(this.props.sorterFilter) && !Object.values(this.props.sorterFilter).every(_ => _ === 'key_a_to_z')) {
        this.props.rows.forEach((ele, index) => {
          let lifter = this.props.sorterFilter[ele];
          switch (lifter) {
            case 'value_a_to_z':
              sortRules.rowOrder.push((a, b) => naturalSort(a[index], b[index]))
              break;
            case 'value_z_to_a':
              sortRules.rowOrder.push((a, b) => -naturalSort(a[index], b[index]))
              break;
            default:
              sortRules.rowOrder.push((a, b) => { return 0 })
          }
        })
        this.props.cols.forEach((ele, index) => {
          let lifter = this.props.sorterFilter[ele];
          switch (lifter) {
            case 'value_a_to_z':
              sortRules.colOrder.push((a, b) => naturalSort(a[index], b[index]))
              break;
            case 'value_z_to_a':
              sortRules.colOrder.push((a, b) => -naturalSort(a[index], b[index]))
              break;
            default:
              sortRules.colOrder.push((a, b) => { return 0 })
          }
        })

      } else {
        sortRules = this.props.aggregatorGather.reduce((pre, cur) => {
          switch (cur.rowOrder) {
            case 'value_a_to_z':
      ...
      this.rowKeys.sort(this.getMutipSort(sortRules['rowOrder']));
      this.colKeys.sort(this.getMutipSort(sortRules['colOrder']));
  ...
  processRecord(record) {
    ...
    this.props.aggregatorGather.forEach((item, index) => {
      const colKey = [];
      const rowKey = [];
      ...
      if (this.props.cols.length) {
        colKey.push(`${item.vals.join(' ')}(${item.aggregatorName})`);
      }
      ...
      if (rowKey.length !== 0) {
        if (!this.rowTotals[[...rowKey, item.aggregatorName].join(String.fromCharCode(0))]) {
          if (this.rowKeys.findIndex(_ => isEqual(_, rowKey)) === -1) {
            this.rowKeys.push(rowKey);
          }
          this.rowTotals[[...rowKey, item.aggregatorName].join(String.fromCharCode(0))] = this.aggregatorGathers[index](this, [...rowKey, item.aggregatorName], [], item.aggregatorName);
        }
        this.rowTotals[[...rowKey, item.aggregatorName].join(String.fromCharCode(0))].push(record);
      }

      if (colKey.length !== 0) {
        if (!this.colTotals[flatColKey]) {
          this.colKeys.push(colKey);
          this.colTotals[flatColKey] = this.aggregatorGathers[index](this, [], colKey, item.aggregatorName);
        }
      ...
      if (colKey.length !== 0 && rowKey.length !== 0) {
        ...
        if (!this.tree[flatRowKey][flatColKey]) {
          this.tree[flatRowKey][flatColKey] = this.aggregatorGathers[index](
            this,
            [...rowKey, item.aggregatorName],
            colKey,
            item.aggregatorName
          );
        }
  ...
  getAggregator(rowKey, colKey, colName) {
    let agg;
    const flatRowKey = rowKey.join(String.fromCharCode(0));
    const flatColKey = colKey.join(String.fromCharCode(0));
    if (rowKey.length === 0 && colKey.length === 0) {
      agg = this.allTotals[colName];
